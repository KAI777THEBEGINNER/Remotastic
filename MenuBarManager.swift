...
class MenuBarManager {
    ...
    // Scroll speed (used for trackpad scroll scale)
    private(set) var scrollSpeed: ScrollSpeed = .medium

    // Cursor speed (trackpad sensitivity)
    private(set) var cursorSpeed: CursorSpeed = .medium
    var onCursorSpeedChanged: ((CursorSpeed) -> Void)?
    ...
    init(statusItem: NSStatusItem) {
        ...
        loadMappings()
        if let savedSpeed = UserDefaults.standard.string(forKey: "cursorSpeed"),
           let speed = CursorSpeed(rawValue: savedSpeed) {
            cursorSpeed = speed
        }
        setupMenuBar()
    }
    ...
    private func rebuildMenu() {
        ...
        let sensitivityItem = NSMenuItem(title: "Trackpad Sensitivity", action: nil, keyEquivalent: "")
        let sensitivitySubmenu = NSMenu()
        for speed in CursorSpeed.allCases {
            let item = NSMenuItem(title: speed.rawValue, action: #selector(changeCursorSpeed(_:)), keyEquivalent: "")
            item.target = self
            item.representedObject = speed
            if cursorSpeed == speed {
                item.state = .on
            }
        ...
        // Pause / Resume
        let pauseTitle = isPaused ? "Resume Mapping" : "Pause Mapping"
        let pauseItem = NSMenuItem(title: pauseTitle, action: #selector(togglePause(_:)), keyEquivalent: "")
        pauseItem.target = self
        menu.addItem(pauseItem)

        if isPaused {
            let noteItem = NSMenuItem(title: "Mapping is paused", action: nil, keyEquivalent: "")
    ...
    func updatePauseStatus(paused: Bool) {
        isPaused = paused
        rebuildMenu()
    }
    ...
    @objc private func changeCursorSpeed(_ sender: NSMenuItem) {
        guard let speed = sender.representedObject as? CursorSpeed else { return }
        cursorSpeed = speed
        UserDefaults.standard.set(speed.rawValue, forKey: "cursorSpeed")
        onCursorSpeedChanged?(speed)
        rebuildMenu()
    }
