...
class TouchHandler {
    ...
    private func findAndStartDevice() {
        ...
        // Prefer Siri Remote (family 0x91 or small surface), otherwise first non-built-in
        var fallbackDev: MTDevice?
        for dev in deviceList {
            if !MTDeviceIsBuiltIn(dev) {
                ...
                print("[TouchHandler] non-built-in candidate: family=0x\(String(family, radix: 16, uppercase: true)) w=\(width) h=\(height)")
                if family == 0x91 || (width < 10000 && height < 10000) {
                    startDevice(dev)
                    return
                }
                if fallbackDev == nil {
