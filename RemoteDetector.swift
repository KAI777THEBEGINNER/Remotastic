...
class RemoteDetector {
    ...
    func startDetection() {
        ...
        guard let manager = manager else {
            print("[RemoteDetector] manager creation failed")
            return
        }
        print("[RemoteDetector] manager created")

        // Match all devices and filter in callbacks (some remotes don't expose vendorID via IOKit matching)
        let matchDict: [String: Any] = [:]
        IOHIDManagerSetDeviceMatching(manager, matchDict as CFDictionary)
        print("[RemoteDetector] matching set to all devices")

        var openResult = IOHIDManagerOpen(manager, IOOptionBits(kIOHIDOptionsTypeNone))
        if openResult != kIOReturnSuccess {
            print("[RemoteDetector] IOHIDManagerOpen (none) failed: \(openResult), trying seize...")
            openResult = IOHIDManagerOpen(manager, IOOptionBits(kIOHIDOptionsTypeSeizeDevice))
        }
        guard openResult == kIOReturnSuccess else {
            print("[RemoteDetector] IOHIDManagerOpen (seize) also failed: \(openResult)")
            return
        }
        print("[RemoteDetector] manager opened successfully")
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
            print("[RemoteDetector] enumerating devices...")
            self?.enumerateAllDevices()
            self?.scheduleRetryEnumeration()
        }
        ...
        wakeObserver = NSWorkspace.shared.notificationCenter.addObserver(
            ...
            print("[RemoteDetector] system woke, re-enumerating devices...")
            self?.enumerateAllDevices()
            self?.scheduleRetryEnumeration()
        }
    }

    private func scheduleRetryEnumeration() {
        ...
        retryTimer = Timer.scheduledTimer(withTimeInterval: 3.0, repeats: true) { [weak self] timer in
            ...
            if self.connectedDeviceCount > 0 {
                print("[RemoteDetector] device already connected, stopping retry")
                timer.invalidate()
                self.retryTimer = nil
                return
            }
            print("[RemoteDetector] retry enumeration (attempt)...")
            self.enumerateAllDevices()
        }
    ...
    private func enumerateAllDevices() {
        guard let manager = manager else {
            print("[RemoteDetector] enumerate: manager is nil")
            return
        }
        let cfSet = IOHIDManagerCopyDevices(manager)
        guard let cfSet = cfSet else {
            print("[RemoteDetector] enumerate: IOHIDManagerCopyDevices returned nil")
            return
        }
        let count = CFSetGetCount(cfSet)
        print("[RemoteDetector] enumerate: found \(count) HID devices")
        guard count > 0 else { return }
        var ptrs = [UnsafeRawPointer?](repeating: nil, count: count)
        CFSetGetValues(cfSet, &ptrs)

        for ptr in ptrs {
            guard let raw = ptr else { continue }
            let device = Unmanaged<IOHIDDevice>.fromOpaque(raw).takeUnretainedValue()
            ...
            print("[RemoteDetector] device: \(name) | vendor=0x\(String(vendor, radix: 16)) | product=0x\(String(product, radix: 16)) | isSiriRemote=\(isSiriRemote(device))")
    ...
    private func isSiriRemote(_ device: IOHIDDevice) -> Bool {
        ...
        if let productName = IOHIDDeviceGetProperty(device, kIOHIDProductKey as CFString) as? String {
            ...
            return name.contains("remote") || name.contains("siri") || name.contains("apple tv") || name.contains("dj7jt0l72330")
        }
